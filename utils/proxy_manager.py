...
class ProxyDense:
    ...
    max_requests: Semaphore = field(default=Semaphore(14))
...
class ProxyManager:
    ...
    def _is_suitable(self, proxy: ProxyDense) -> bool:
    ...
    def add_proxy(
        ...
        if self._is_suitable(proxy):
            self.proxies.append(proxy)
        else:
            raise ConnectionError(
                f"Bad proxy {proxy.ip_adress}. "
                "Haven't connected to https://httpbin.io/ip."
            )
